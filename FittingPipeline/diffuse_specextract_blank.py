...
def main_extract(chandra_dir,source_dir,OBSIDS,source_reg):
    ...
    for obsid in OBSIDS:
        ...
        evt_file = None  # Check if reprojected event file is available
        for fname in os.listdir('.'):
            if fname.endswith('_reproj_evt.fits'):
                evt_file = obsid+"_reproj_evt.fits"
        if evt_file == None:  # If not, get normal event file
            if len(obsid) == 3:
